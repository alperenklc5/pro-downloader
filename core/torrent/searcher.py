...
CATEGORY_GAMES = [4000, 4010, 4020, 4030, 4040, 4050, 4060, 4070, 4080]

TRUSTED_GAME_GROUPS = ["fitgirl", "dodi", "kaos", "corepack", "gog", "razor1911"]
...
def _parse_result(item: dict, category: str = "") -> TorrentResult | None:
    """Jackett sonucunu TorrentResult'a çevir. Geçersiz magnet/URL varsa None döner."""
    ...
    if not magnet_valid and not url_valid:
        return None
    ...
    if category:
        ...
        result_category = "movie" if any(c in CATEGORY_MOVIES for c in item_cats) else "series"
...
def search_movies(
    ...
    results = [r for item in items if (r := _parse_result(item)) is not None]
...
def _sort_game_results(results: list[TorrentResult]) -> list[TorrentResult]:
    """Güvenilir repack gruplarını öne al, sonra seed sayısına göre sırala."""
    def score(r: TorrentResult) -> int:
        group_bonus = 100 if any(g in r.title.lower() for g in TRUSTED_GAME_GROUPS) else 0
        return group_bonus + r.seeds
...
def search_games(
    ...
    """PC oyun torrenti ara."""
    items = _jackett_search(query, CATEGORY_GAMES, jackett_url, jackett_key)
    results = [r for item in items if (r := _parse_result(item, "game")) is not None]
    results = [r for r in results if r.seeds > 0]
    return _sort_game_results(results)[:50]
